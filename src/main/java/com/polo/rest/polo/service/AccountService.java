...
public class AccountService {
	...
	private ConvertionManager convartionManagerInstance;
	...
	public AccountService() {
		convartionManagerInstance = ConvertionManager.getConvertionManager();
	}
	
	public void createAccount( AccountDto accountDto ) throws AccountException {
		accountValidator.validateAccount(accountDto);
		Account accountEntity = convartionManagerInstance.convertAccountToEntity( accountDto );
		checkAccountExists(accountEntity, CREATE);
		accountDao.createAccount( accountEntity );
		List<Parent> parentsEntityList = convartionManagerInstance.convertParentsDtoToEntity( accountDto.getParentsDtoList(), accountEntity );
	...
	public AccountDto getAccountByCardId( int cardId ) throws AccountException {
		...
		AccountDto accountDto = convartionManagerInstance.convertAccountToDto( accountEntity );
		List<ParentsDto> parentDtoList = convartionManagerInstance.convertParentsToDto( parentList );
	...
	public void updateAccount(AccountDto accountDto) throws AccountException {
		...
		List<Parent> parentEntityList = convartionManagerInstance.convertParentsDtoToEntity( accountDto.getParentsDtoList(), accountEntity );
	...
	private List<AccountDto> convertAccountEntityToDtoListWithParents(List<Account> accountEntityList) {
		...
		for ( Account acc : accountEntityList ) {
			...
			List<ParentsDto> parentDtoList = convartionManagerInstance.convertParentsToDto( parentEntityList );
			AccountDto accountDto = convartionManagerInstance.convertAccountToDto(acc);
			accountDto.setParentsDtoList( parentDtoList );
